// prevent error associated with print routine if the page is not inside an iframe
try {
	// set this attribute to allow printing of the iframe in MyLO rather than the whole MyLO environment.
...
// ctrl-P captured from parent frame
        ...
        $(window.top.document).keydown(function (event) {
            ...
            function printContent() {
				console.log('window.top.document - printContent called');
				try{
				...
				// set the contentframe to be the document if the document is not loaded inside an iframe
					window.print();
					$(".pageTitlePrint").remove();
				}
            }
...
// ctrl-P captured inside iframe
        ...
        $(document).keydown(function (event) {
            ...
            function printContent() {
				console.log(' document - printContent called');
				try{
					var contentframe = $('iframe', window.top.document)[0];
					var pageTitle=$(".d2l-page-main .d2l-page-header .d2l-page-title-c form:first-of-type > label",window.top.document).text();
					...
					// don't need to print because window.top.document has already been called.
					$(".pageTitlePrint").remove();
				}
            }
        });
        ...
        $('.pdf-link').click(function () {
			console.log('Print this page link clicked');
            //set contentframe to the iframe if the page is hosted in an iframe, otherwise print the whole page
			try{
				var contentframe = $('iframe', window.top.document)[0];
	            var pageTitle=$(".d2l-page-main .d2l-page-header .d2l-page-title-c form:first-of-type > label",window.top.document).text();
				pageTitle="<div class='col-xs-12 pageTitlePrint'><h2>"+pageTitle+"</h2></div>";
        ...
        $('.pdf-link').html('Print/Download this page');
